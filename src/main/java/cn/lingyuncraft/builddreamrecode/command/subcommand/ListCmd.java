        ...
        switch (args.length) {
            case 1:
                if (!(sender instanceof Player)) {
                    sender.sendMessage(plugin.lang.build(plugin.localeKey, I18n.Type.ERROR, "在控制台中不能查询自己的梦境."));
                } else {
                    Player user = (Player) sender;
                    I18n.send(user, plugin.lang.build(plugin.localeKey, I18n.Type.WARN, "&a&l您&7的已筑梦境列表"));
            ...
            case 2:
                if (!sender.hasPermission("Builddream.admin.check")) {
                    ...
                    if (target == null) {
                        sender.sendMessage(plugin.lang.build(plugin.localeKey, I18n.Type.WARN, "目标玩家离线或不存在."));
                    } else {
                        sender.sendMessage(plugin.lang.build(plugin.localeKey, I18n.Type.WARN, "&a&l" + target.getName() + " &7的已筑梦境列表"));
                        for (Dream dream : Storage.get().getDataMap().values()) {
                ...
                break;
            default:
                sender.sendMessage(plugin.lang.build(plugin.localeKey, I18n.Type.ERROR, "参数有误, 请检查参数是否正确."));
                break;
        }
        return true;
    }
